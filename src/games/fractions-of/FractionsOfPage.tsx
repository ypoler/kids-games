...
import { nextOfProblem, type OfProblem } from './engine'
...
export function FractionsOfPage() {
  const { recordRoundBest, enqueueSession, setLastGame } = useStore()
  const { profile, progress, general, fractionsOf } = useActiveProfile()
  const round = fractionsOf.round
  const timed = round.type === 'timed'
  const maxD = fractionsOf.max
  const sessionLength = round.type === 'questions' ? round.count : 0

  const [started, setStarted] = useState(false)
  const [problem, setProblem] = useState<OfProblem | null>(null)
  ...
  const completeRound = (s: RunScore, durationSec: number) => {
    ...
    prevBestRef.current = bestFor(resolvedBests(progress, 'fractions-of'), round)
    recordRoundBest('fractions-of', round, s.correct)
    enqueueSession({
      player: profile.name,
      game: 'fractions-of',
      mode: timed ? 'of+timed' : 'of',
  ...
  useEffect(() => {
    setLastGame('fractions-of')
  }, [setLastGame])

  const resetAids = (total: number) => {
    setPainted(Array.from({ length: total }, () => false))
    setCuts(Array.from({ length: Math.max(0, total - 1) }, () => false))
  }

  const pick = useCallback(
    (prev: OfProblem | null) => {
      const next = nextOfProblem(maxD, prev)
      setProblem(next)
      setAnswer('')
      setFeedback(null)
      resetAids(next.total)
    },
    [maxD],
  )
  ...
  const submit = () => {
    ...
    if (!answer.length || Number.isNaN(n)) return
  ...
  useEffect(() => {
    if (!started || runDone || feedback) return
  ...
  }, [started, runDone, feedback, answer, problem])

  if (!started || !problem) {
    return (
      <Shell title={t.fracOf} dir="rtl">
  ...
  return (
    <Shell title={t.fracOf} dir="rtl">
      ...
      <p className="frac-prompt frac-prompt-one" aria-live="polite">
        {t.fracOfHave} {problem.total} {t.fracOfItems}. {t.fracOfAsk}{' '}
        <Frac n={problem.n} d={problem.d} />?
      </p>
      <ObjectLine
        total={problem.total}
        painted={painted}
        cuts={cuts}
        onTogglePaint={(i) => setPainted((p) => p.map((v, j) => (j === i ? !v : v)))}
        onToggleCut={(i) => setCuts((c) => c.map((v, j) => (j === i ? !v : v)))}
      />
